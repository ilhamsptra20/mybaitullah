...
const WalkieTalkieRoom = ({ navigation, route }) => {
    const width = Dimensions.get('window').width;
    const height = Dimensions.get('window').height;

    // API
    const { idRoom, role, channelName, travelName, imageUser, hostName, periodDate, periodTime, channelCode, passCode, isOnline } = route.params; // role = "host" atau "audience"
    ...
    const channel_online = isOnline ? true : false;
    const [statusChannel, setStatusChannel] = useState(channel_online);
        ...
        try {
            ...
            agoraEngine.registerEventHandler({
                ...
                onUserOffline: (_connection, Uid) => {
                    ...
                    if (role !== 'host') {
                        console.log("Host telah meninggalkan channel. Audience akan keluar.");
                        Alert.alert("Host telah meninggalkan siaran", "Kamu akan keluar otomatis.");
                        leave(); // Fungsi kamu yang untuk keluar dari channel
                    }
                },
    ...
    const join = async () => {
        ...
        const joinChannelApp = await joinChannel(idRoom, PASS_CODE);

        if (!joinChannelApp.success) {
            Alert.alert("Gagal bergabung:", joinChannelApp.error || "Unknown error");
            return;
        } else {
            console.log("Join Channel:", idRoom, CHANNEL_NAME);
    ...
    const leave = async () => {
        try {
            const leaveChannelApp = await leaveChannel(idRoom);
            if (!leaveChannelApp.success) {
                Alert.alert("Gagal keluar:", leaveChannelApp.error || "Unknown error");
                return;
            }
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'position'} style={{ flex: 1 }} >
                ...
                <View style={[BaseStyle.absolute, BaseStyle.index1, BaseStyle.wFull, BaseStyle.BgTrasnparent, ({ paddingTop: StatusBar.currentHeight + 10, paddingHorizontal: 14, paddingBottom: 10, borderBottomLeftRadius: 6, borderBottomRightRadius: 6 })]}>
                    <View style={[BaseStyle.row, BaseStyle.justifyBetween, BaseStyle.alignItemsCenter, BaseStyle.pb5]}>
                        ...
                        <Text style={[BaseStyle.MaisonBold, BaseStyle.textWhite, BaseStyle.textMD]}>Walkie Talkie</Text>
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <View style={[BaseStyle.wFull, BaseStyle.mb20, BaseStyle.p10, BaseStyle.radius20, BaseStyle.borderGray100, ({ flex: 1, paddingBottom: 0 })]}>
                                <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={true} indicatorStyle="#33C060" edgeEffectColor={Platform.OS === 'android' ? '#33C060' : undefined} scrollIndicatorInsets={{ right: 1 }} contentContainerStyle={{ flexGrow: 1, justifyContent: 'flex-end' }}>
                                    <Text style={[BaseStyle.MaisonDemi, BaseStyle.textBlack, BaseStyle.textXS1, BaseStyle.lh18, BaseStyle.mb10]}>Assalamu’alaikum, selamat datang di Walkie Talkie! Semoga sesi ini membawa manfaat untuk kita semua.</Text>
                        ...
                        {role === 'host' ? (
                            <View>
                                <View style={[BaseStyle.row, BaseStyle.justifyBetween, BaseStyle.alignItemsCenter, BaseStyle.p10, BaseStyle.BgLightGreen200, ({ borderRadius: 100 })]}>
                                    <Button text={isJoined === true || statusChannel === true ? "Akhiri Siaran" : "Mulai Siaran"} color="#FFFFFF" backgroundColor={isJoined === true || statusChannel === true ? "#FF4D4D" : "#208D33"} borderRadius={50} width='82%' paddingVertical={typeof menu === "number" ? menu * 0.30 : 10} onPress={isJoined === true || statusChannel === true ? openBottomSheetLeaveChannel : openBottomSheetJoinChannel} />
                            ...
                            <View>
                                <View style={[BaseStyle.row, BaseStyle.justifyBetween, BaseStyle.alignItemsCenter, BaseStyle.p10, BaseStyle.BgLightGreen200, ({ borderRadius: 100 })]}>
                                    <Button text={isJoined === true || statusChannel === true ? "Keluar dari Siaran" : "Gabung Siaran"} color="#FFFFFF" backgroundColor={isJoined === true || statusChannel === true ? "#FF4D4D" : "#208D33"} borderRadius={50} width='100%' paddingVertical={typeof menu === "number" ? menu * 0.30 : 10} onPress={isJoined === true || statusChannel === true ? openBottomSheetLeaveChannel : openBottomSheetJoinChannel} />
                                </View>
                            </View>
                        )
                        }
...
export default WalkieTalkieRoom;

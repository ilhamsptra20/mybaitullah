...
// ICON
import {IconArrowLeftWhite, IconCaretRightGreen, IconCartGreen, IconCartOffGreen, IconFlagGreen, IconPaymentGreen, IconUserGreen} from '../../assets/icon'
...
const data = [
  ...
  },
  {
    ...
    listItem: [
      ...
      {
        text: 'Anda dapat mengatur metode pembayaran Anda di halaman pemesanan',
...
const AdviceAndHelpDetail = ({navigation}) => {
  const route = useRoute();
  const { title, text, listItem, image } = route.params;
        ...
        <View style={[BaseStyle.relative, ({top: headerHeight * -0.4})]}>
          <View style={[BaseStyle.wrap]}>
            <View style={[BaseStyle.BgWhite, BaseStyle.shadow, BaseStyle.p20, BaseStyle.radius10, BaseStyle.mb20]}>
              <Text style={[BaseStyle.textLG, BaseStyle.MaisonBold, BaseStyle.mb10]}>{title}</Text>

              <Text style={[BaseStyle.textBase, BaseStyle.textBlack, BaseStyle.lh20, BaseStyle.mb20]}>{text}</Text>

              {listItem && listItem.length > 0 ? (
                <View style={[BaseStyle.mt10]}>
                  {listItem.map((item, index) => (
                    <View key={index} style={[BaseStyle.mb20]}>
                      <Text style={[BaseStyle.textBase, BaseStyle.textGray300, BaseStyle.lh20, BaseStyle.mb10]}>{`${index + 1}. ${item.text}`}</Text>
                      {item.image && (
                        <Image source={item.image} style={[BaseStyle.wFull, BaseStyle.hAuto, BaseStyle.potrait]} resizeMode="contain" />
                      )}
                    </View>
                  ))}
                </View>
                ...
                image && (
                  <Image source={image} style={[BaseStyle.wFull, BaseStyle.hAuto, BaseStyle.potrait]} resizeMode="contain" />
                )
              )}
            </View>
          </View>
...
export default AdviceAndHelpDetail
